...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `/seo-audit-cases/${slug}/` },
    robots: { index: !c.frontmatter.noIndex, follow: true },
    openGraph: {
      title: c.frontmatter.title,
      description: c.frontmatter.description,
      images: c.frontmatter.featuredImage ? [{ url: c.frontmatter.featuredImage }] : [],
    },
  };
}
...
export default async function CaseDetailPage({ params }: Props) {
  ...
  const mdxSource = await serializeMdx(caseItem.content);
    ...
    <>
      ...
      <article className="container-content py-12 lg:py-16">
        ...
        <div className="flex flex-wrap gap-3 mb-3">
          {caseItem.frontmatter.industry && <span className="text-xs font-semibold uppercase tracking-wider text-[var(--color-accent)] bg-[var(--color-bg-secondary)] px-2 py-1 rounded">{caseItem.frontmatter.industry}</span>}
          {caseItem.frontmatter.websiteType && <span className="text-xs font-semibold uppercase tracking-wider text-[var(--color-text-muted)] bg-[var(--color-bg-secondary)] px-2 py-1 rounded">{caseItem.frontmatter.websiteType}</span>}
        </div>

        <h1 className="text-3xl lg:text-4xl font-bold font-[family-name:var(--font-heading)] text-balance leading-tight">{caseItem.frontmatter.title}</h1>
        <p className="mt-4 text-lg text-[var(--color-text-secondary)]">{caseItem.frontmatter.description}</p>

        <div className="mt-4 flex flex-wrap gap-4 text-sm text-[var(--color-text-muted)]">
          {caseItem.frontmatter.auditDate && <time dateTime={caseItem.frontmatter.auditDate}>Audit Date: {new Date(caseItem.frontmatter.auditDate).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</time>}
          <span>{rt} min read</span>
        </div>

        {caseItem.frontmatter.featuredImage && (
          <figure className="mt-8 -mx-4 lg:-mx-8">
            <img src={caseItem.frontmatter.featuredImage} alt={caseItem.frontmatter.featuredImageAlt || caseItem.frontmatter.title} className="w-full rounded-lg" />
            {caseItem.frontmatter.featuredImageAlt && <figcaption className="mt-2 text-center text-sm text-[var(--color-text-muted)]">{caseItem.frontmatter.featuredImageAlt}</figcaption>}
          </figure>
        )}
        ...
        {/* Metrics */}
        {caseItem.frontmatter.metrics.length > 0 && (
          <section className="mt-10">
            <h2 className="text-xl font-bold font-[family-name:var(--font-heading)] mb-4">Metrics</h2>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {caseItem.frontmatter.metrics.map((m, i) => {
                const sentimentColor = m.sentiment === "positive" ? "text-[var(--color-success)]" : m.sentiment === "negative" ? "text-[var(--color-critical)]" : "text-[var(--color-text)]";
                const arrow = m.direction === "up" ? "↑" : m.direction === "down" ? "↓" : "";
                return (
                  <div key={i} className="border border-[var(--color-border)] rounded-lg p-5">
                    <p className="text-sm text-[var(--color-text-muted)]">{m.label}</p>
                    <p className={`text-2xl font-bold ${sentimentColor}`}>{m.value}{m.change && <span className="text-sm ml-2">{arrow} {m.change}</span>}</p>
              ...
              {relatedCases.map((rc) => (
                <Link key={rc.slug} href={`/seo-audit-cases/${rc.slug}/`} className="group block border border-[var(--color-border)] rounded-lg overflow-hidden hover:shadow-[var(--shadow-card-hover)] transition-shadow">
                  <div className="aspect-video bg-[var(--color-bg-secondary)] flex items-center justify-center">
                    {rc.featuredImage ? <img src={rc.featuredImage} alt={rc.featuredImageAlt || rc.title} className="w-full h-full object-cover" loading="lazy" /> : <span className="text-[var(--color-text-muted)] text-xs">Image</span>}
                  </div>
                  <div className="p-4">
