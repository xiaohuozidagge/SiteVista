...
export default async function BlogPostPage({ params }: Props) {
  ...
  if (relatedPosts.length === 0) {
    const sameCategory = getAllPosts()
      .filter(
        (p) =>
  ...
  return (
    <>
      ...
      {post.frontmatter.faq.length > 0 && (
        <FAQPageSchema questions={post.frontmatter.faq} />
      )}
      ...
      <article className="container-content py-12 lg:py-16">
        ...
        <Link
          href={`/${post.frontmatter.category}/`}
          className="text-sm font-semibold uppercase tracking-wider text-[var(--color-accent)]"
        >
          {post.frontmatter.categoryName || post.frontmatter.category}
        </Link>
        ...
        {post.frontmatter.featuredImage && (
          <figure className="mt-8 -mx-4 lg:-mx-8">
            <img
              ...
              alt={
                post.frontmatter.featuredImageAlt || post.frontmatter.title
              }
        ...
        {post.frontmatter.tags.length > 0 && (
          <div className="mt-10 flex flex-wrap gap-2">
            {post.frontmatter.tags.map((tag) => (
              <span
                key={tag}
                className="px-3 py-1 text-xs font-medium bg-[var(--color-bg-secondary)] text-[var(--color-text-secondary)] rounded-full"
              >
                {tag}
              </span>
            ))}
            ...
            <div className="space-y-2">
              {post.frontmatter.faq.map((item, i) => (
                ...
                >
                  ...
                  <div className="px-5 pb-4 text-[var(--color-text-secondary)]">
                    {item.answer}
                  </div>
                </details>
              ))}
          ...
          <section className="mt-12 border-t border-[var(--color-border)] pt-10">
            <div className="flex items-start gap-4 p-6 bg-[var(--color-bg-secondary)] rounded-lg">
              <div className="w-14 h-14 rounded-full bg-[var(--color-border)] flex items-center justify-center text-lg font-bold text-[var(--color-text-muted)] shrink-0">
                {author.frontmatter.name.charAt(0)}
              </div>
              <div>
                <p className="font-bold font-[family-name:var(--font-heading)]">
                  {author.frontmatter.name}
                </p>
                {author.frontmatter.jobTitle && (
